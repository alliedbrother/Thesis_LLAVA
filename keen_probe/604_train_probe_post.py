...
def load_embeddings_and_scores(h5_folder, req_embeddings, labels_dict, split_ids):
                    ...
                    try:
                        ...
                        for part in path_parts:
                            if part not in group:
                                raise KeyError(f"Missing group '{part}' in {image_id}")
                            group = group[part]
                        embedding = np.array(group)
                        # Squeeze and check shape if needed
                        embedding = np.squeeze(embedding)
                        all_embeddings.append(embedding)
                        all_scores.append(labels_dict[image_id])
                        processed_images += 1
    ...
    return np.array(all_embeddings), np.array(all_scores)
...
def load_embeddings_and_scores(h5_folder, req_embeddings, labels_dict, split_ids):
                ...
                for image_group in f.keys():
                    ...
                    if image_group in split_ids:
                        try:
                            ...
                            # Check each part of the path exists before descending
                            for path_part in path_parts:
                                if path_part not in current:
                                    raise KeyError(f"Missing group '{path_part}' in {image_group}")
                                current = current[path_part]
                            # Get the embeddings
                            emb = current[:]
                            emb = np.squeeze(emb)  # Squeeze to remove extra dimensions
    ...
    print(f"Processed images: {processed_images}")
    print(f"Ignored images (errors): {ignored_images}")
