...
CONFIG = {
    ...
    "MODEL_SAVE_DIR": "/root/project/Thesis_LLAVA/keen_probe/keen_data/models_binary_paligemma",
    "LOG_DIR": "/root/project/Thesis_LLAVA/keen_probe/keen_data/logs_binary_paligemma",
    
    # Model parameters
    "REQ_EMBEDDINGS": "vision_embeddings",
    "INPUT_DIM": 1152,        # Length of input embeddings
...
def load_embeddings_and_scores(h5_folder, req_embeddings, labels_dict, split_ids):
    ...
    total_images = 0
    total_matched = 0
    path_parts = req_embeddings.split('/')
                ...
                for image_group in f.keys():
                    ...
                    if image_group in split_ids:
                        try:
                            ...
                            # Get the embeddings and squeeze to 1D
                            emb = np.array(current[:]).squeeze()
                            if emb.ndim != 1 or emb.shape[0] != CONFIG["INPUT_DIM"]:
                                logging.warning(f"Unexpected embedding shape {emb.shape} for {image_group}, skipping.")
                                continue
...
def train_model(X_train, y_train, X_val, y_val, hyperparams):
    """Train the model with validation monitoring and learning rate scheduling (binary classification)."""
...
def evaluate_model(model, X, y, split_name):
    """Evaluate model performance on a dataset (binary classification)."""
    model.eval()
    with torch.no_grad():
...
def main():
    ...
    for i, hyperparams in enumerate(hyperparam_combinations, 1):
        ...
        try:
            ...
            if val_metrics['roc_auc'] > best_val_loss:
                best_val_loss = val_metrics['roc_auc']
                best_model = model
                ...
                logging.info(f"Validation ROC-AUC: {best_val_loss:.6f}")
